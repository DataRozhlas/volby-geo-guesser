  ...
  var Loader = {
    ...
    load: function (key_, what_, callback) {
      ...
      if (
        what.api == "full" &&
        !window.SMap &&
        list.indexOf(files["api-jak"][0]) == -1
      ) {
      ...
      if (
        what.poi &&
        !(window.SMap && window.SMap.Detail) &&
        list.indexOf(files["api-poi"][2]) == -1 &&
      ...
      if (
        what.suggest &&
        !(window.SMap && window.SMap.Suggest) &&
        list.indexOf(files["api-jak-poi-pano-suggest"][2]) == -1
      ) {
      ...
      list.push(
        "/js/lang/" + (langs[this.lang] ? langs[this.lang] : langs.cs) + ".js"
      );
      ...
      while (css.length) {
        ...
        link.href =
          ...
          css.shift() +
          "?v" +
          (this.version == 0 ? Math.random() : this.version);
    ...
    _loadSync: function (list) {
      for (var i = 0; i < list.length; i++) {
        document.write(
          `<script charset="utf-8" type="text/javascript" src="${list[i]}"></script>`
        );
      }
    },
